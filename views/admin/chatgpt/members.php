    ...
    <div class="container-fluid">
        ...
        <div class="card custom-card">
            <div class="card-header gptb-card-header">
                <span class="gptb-title-with-bar">THÀNH VIÊN FARM</span>
            </div>
                ...
                <form method="get" class="row align-items-end" id="filterForm">
                    ...
                    <div class="col-lg-2 col-md-4 mb-2">
                        <div class="gptb-filter-actions">
                            <a href="<?= url('admin/gpt-business/members') ?>" class="btn btn-danger btn-sm w-100">
            ...
            <div class="card-body gptb-orders-card-body">
                                    ...
                                    <tr>
                                        <td colspan="8" class="text-center gptb-orders-empty">
                                            <div class="gptb-orders-empty-box">
                                                ...
                                                <p class="gptb-orders-empty-text">Thử thay đổi bộ lọc để tìm kiếm thành
                                                    viên.</p>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <?php foreach ($members as $member): ?>
                                        <tr>
                                            <td class="text-center">
                                                <span
                                                    class="gptb-email"><?= htmlspecialchars($member['email'] ?? '--') ?></span>
                                            </td>
                                            <td class="text-center">
                                                <span
                                                    class="cat-badge"><?= htmlspecialchars($member['farm_name'] ?? '--') ?></span>
                                            </td>
                                            <td class="text-center">
                                                <span
                                                    class="<?= htmlspecialchars($member['role_badge_class'] ?? 'gptb-badge gptb-badge--primary') ?>">
                                            ...
                                            <td class="text-center">
                                                <span
                                                    class="<?= htmlspecialchars($member['status_badge_class'] ?? 'gptb-badge gptb-badge--muted') ?>">
                                                    <?= htmlspecialchars($member['status_label'] ?? '--') ?>
                                                </span>
                                            </td>
                                            <td class="text-center">
                                                <span
                                                    class="date-badge"><?= htmlspecialchars($member['first_seen_display'] ?? '--') ?></span>
                                            </td>
                                            <td class="text-center">
                                                <span
                                                    class="date-badge"><?= htmlspecialchars($member['last_seen_display'] ?? '--') ?></span>
                                            </td>
                                            <td class="text-center">
                ...
                <div class="gptb-orders-table-footer">
                    <span>Hiển thị <strong><?= count($members) ?></strong> thành viên trong danh sách hiện tại</span>
                    <span>Bảng snapshot thành viên GPT Business</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function () {
        ...
        document.addEventListener('click', function (event) {
            ...
            fetch('<?= url('admin/gpt-business/members/remove/') ?>' + memberId, {
                method: 'POST',
