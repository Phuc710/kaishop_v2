...
<head>
    ...
    <title>Lịch Sử Mua Code | <?=$chungapi['ten_web'];?></title>
...
// Đếm tổng bản ghi
$total_query = mysqli_query($connection, "SELECT COUNT(*) AS total FROM `lich_su_mua_code` WHERE `username` = '$username'");
$total_row = mysqli_fetch_assoc($total_query);
$total_records = $total_row['total'];
$total_pages = ceil($total_records / $limit);

// Lấy dữ liệu phân trang
$result = mysqli_query($connection, "SELECT * FROM `lich_su_mua_code` WHERE `username` = '$username' ORDER BY `id` DESC LIMIT $start, $limit");
$i = $start + 1;
?>
<div class="row">
    <div class="col-md-12">
                ...
                <table class="w-100 dashboard-table table text-nowrap">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-sm text-dark"><p class="text-nowrap"><?=$row['trans_id'];?></p></td>
                            <td class="text-sm text-dark"><p class="text-nowrap"><?=$code['title'];?></p></td>
                            <td class="text-sm text-dark"><p class="text-nowrap"><?=ngay($row['time']);?></p></td>
                            <td class="text-sm text-dark"><p class="text-nowrap"><?=code($row['status']);?></p></td>
                            <td class="text-nowrap">
                                <button onclick="location.href='<?= $code['link']; ?>';" class="btn btn-dark btn-sm">Download</button>
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
