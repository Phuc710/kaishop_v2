        ...
        window.jQuery.ajaxPrefilter(function (options, originalOptions, jqXHR) {
            ...
            if (typeof options.data === 'string') {
                if (options.data.indexOf('csrf_token=') === -1) {
                    options.data = options.data + (options.data ? '&' : '') + 'csrf_token=' + encodeURIComponent(token);
                }
                return;
            }
            ...
            if (originalOptions.data && typeof originalOptions.data === 'object' && !Array.isArray(originalOptions.data)) {
                ...
                // jQuery prefilter runs after jQuery may have already serialized `options.data`.
                // Do not overwrite stringified payload with a plain object (causes `.replace` crash).
            ...
            if (typeof options.data === 'undefined' || options.data === null || options.data === '') {
