...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  `visibility_mode` enum('both','web','telegram','hidden') NOT NULL DEFAULT 'both' COMMENT 'both=web+telegram, web=chi web, telegram=chi telegram, hidden=an ca 2',
  `show_on_web` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1 = hien tren web',
  `show_on_telegram` tinyint(1) NOT NULL DEFAULT 1 COMMENT '1 = hien tren Telegram',
  ...
  KEY `idx_products_visibility_mode` (`visibility_mode`),
  KEY `idx_products_show_on_web` (`show_on_web`),
  KEY `idx_products_show_on_telegram` (`show_on_telegram`),
