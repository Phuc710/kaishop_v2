...
$host = $connection->query("SELECT * FROM `list_host` WHERE `id` = '$goi' ")->fetch_array();
$discount_data = $connection->query("SELECT * FROM `gift_code` WHERE `type` = 'host' ")->fetch_array();
// Kiểm tra 
...
if ($username == "") {
    ...
    $sv_host = $connection->query("SELECT * FROM `list_server_host` WHERE `id` = '$server_host' ")->fetch_array();
    ...
    if ($user['money'] < $tongtien) {
        ...
        $sql = $connection->query("INSERT INTO `lich_su_mua_host` SET 
            `username`    = '$username',
            `domain`      = '$domain',
            `goi_host`    = '$goi_host',
