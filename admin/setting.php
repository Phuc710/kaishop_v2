...
if (!class_exists('MaintenanceService')) {
    require_once dirname(__DIR__) . '/app/Services/MaintenanceService.php';
}
$maintenanceService = new MaintenanceService($connection);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_SESSION['admin'])) {
    ...
    if ($_POST['action'] === 'update_maintenance') {
        $result = $maintenanceService->saveConfig([
            'maintenance_enabled' => $_POST['maintenance_enabled'] ?? '',
            'maintenance_start_at' => $_POST['maintenance_start_at'] ?? '',
            'maintenance_duration_minutes' => $_POST['maintenance_duration_minutes'] ?? 60,
            'maintenance_notice_minutes' => $_POST['maintenance_notice_minutes'] ?? 5,
            'maintenance_message' => $_POST['maintenance_message'] ?? '',
        ]);
        echo json_encode([
            'status' => !empty($result['success']) ? 'success' : 'error',
            'message' => $result['message'] ?? 'KhÃ´ng thá»ƒ lÆ°u cáº¥u hÃ¬nh báº£o trÃ¬'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    if ($_POST['action'] === 'clear_maintenance') {
        $result = $maintenanceService->clearNow();
        echo json_encode([
            'status' => !empty($result['success']) ? 'success' : 'error',
            'message' => $result['message'] ?? 'KhÃ´ng thá»ƒ káº¿t thÃºc báº£o trÃ¬'
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }
}

$maintenanceConfig = $maintenanceService->getConfig();
$maintenanceState = $maintenanceService->getState(true);
$maintenanceStartInput = '';
if (!empty($maintenanceConfig['start_at'])) {
    $maintenanceStartTs = strtotime((string) $maintenanceConfig['start_at']);
    if ($maintenanceStartTs !== false) {
        $maintenanceStartInput = date('Y-m-d\\TH:i', $maintenanceStartTs);
    }
}
?>
<!DOCTYPE html>
...
<body class="hold-transition sidebar-mini layout-fixed">
                ...
                <div class="container-fluid">
                    ...
                    <div class="row lg-12">
                        <section class="col-lg-6 connectedSortable">
                            ...
                            <form id="form-maintenance" action="" method="POST" class="mt-3">
                                <div class="card card-primary card-outline">
                                    <div class="card-header">
                                        <h3 class="card-title">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group mb-4">
                                            <div class="custom-control custom-switch">
                                                <input type="checkbox" class="custom-control-input" id="maintenance_enabled" name="maintenance_enabled" value="1" <?= !empty($maintenanceConfig['enabled']) ? 'checked' : '' ?>>
                                                <label class="custom-control-label font-weight-bold" for="maintenance_enabled">BẢO TRÌ HỆ THỐNG</label>
                                            </div>
                                            <small class="text-muted d-block mt-1">
                                                <i class="fas fa-info-circle mr-1"></i>
                                                Khi công tắc <b>Tắt</b>: Hết thời lượng sẽ tự hoạt động lại. Khi công tắc <b>Bật</b>: Thời lượng bên dưới chỉ mang tính chất dự kiến.
                                            </small>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="datetime-local" class="form-control" name="maintenance_start_at" value="<?= htmlspecialchars($maintenanceStartInput) ?>">
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Thời lượng bảo trì</label>
                                                    <input type="number" class="form-control" name="maintenance_duration_minutes" min="1" max="10080" value="<?= (int) ($maintenanceConfig['duration_minutes'] ?? 60) ?>">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Countdown trước bảo trì (phút)</label>
                                                    <input type="number" class="form-control" name="maintenance_notice_minutes" min="1" max="60" value="<?= (int) ($maintenanceConfig['notice_minutes'] ?? 5) ?>">
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Thời gian kết thúc (dự kiến)</label>
                                                    <input type="text" id="maintenance_end_preview" class="form-control" readonly value="<?= !empty($maintenanceState['end_at']) ? htmlspecialchars((string) $maintenanceState['end_at']) : '' ?>">
                                                </div>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label>Nội dung bảo trì (hiển thị trên trang bảo trì)</label>
                                            <textarea class="form-control" name="maintenance_message" rows="4" placeholder="Nhập nội dung thông báo cho người dùng..."><?= htmlspecialchars((string) ($maintenanceConfig['message'] ?? '')) ?></textarea>
                                        </div>


                                        <div class="card-footer clearfix px-0 pb-0">
                                            <button type="submit" class="btn btn-info btn-icon-left m-b-10">
                                                <i class="fas fa-save mr-1"></i>Lưu cấu hình bảo trì
                                            </button>
                                            <button type="button" id="btn-clear-maintenance" class="btn btn-danger btn-icon-left m-b-10 ml-2">
                                                <i class="fas fa-trash-alt mr-1"></i>Kết thúc
                                            </button>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            handleFormSubmit('form-maintenance', 'update_maintenance', function () {
                setTimeout(function () {
            ...
            $('#btn-clear-maintenance').on('click', function () {
                ...
                btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin mr-1"></i> Äang xá»­ lÃ½...');

                $.ajax({
                    url: '',
                    type: 'POST',
                    dataType: 'json',
                    data: { action: 'clear_maintenance' },
                    success: function (response) {
                        btn.prop('disabled', false).html(originalText);
                        Toast.fire({
                            icon: response && response.status === 'success' ? 'success' : 'error',
                            title: (response && response.message) ? response.message : 'CÃ³ lá»—i xáº£y ra!'
                        });
            ...
            function updateMaintenanceEndPreview() {
                const startValue = $('input[name="maintenance_start_at"]').val();
                const durationValue = parseInt($('input[name="maintenance_duration_minutes"]').val(), 10) || 0;
                const $preview = $('#maintenance_end_preview');

                if (!startValue || durationValue <= 0) {
                    $preview.val('');
                    return;
                }
                ...
                $preview.val(formatDateTime(end));
            }

            $('input[name="maintenance_start_at"], input[name="maintenance_duration_minutes"]').on('change input', updateMaintenanceEndPreview);
            updateMaintenanceEndPreview();
        });
    </script>
</body>
